...
from elfesteem.cstruct import Constants, CStruct, CStructWithStrTable
...
class Shdr(CStructWithStrTable):
    ...
    def type_txt(self):
        ...
        if 'SHT_'+m in constants:
            ret = m+'_'+constants['SHT_'+m][self.type]
...
no_name=('SHN_LORESERVE','SHN_HIRESERVE',
         'SHN_LOPROC','SHN_HIPROC','SHN_LOOS','SHN_HIOS'),
)
...
no_name = ('SHT_LOSUNW', 'SHT_HISUNW', 'SHT_HIOS'),
)
...
no_name = ('PT_LOOS', 'PT_LOSUNW', 'PT_HISUNW', 'PT_HIOS', 'PT_LOPROC', 'PT_HIPROC')
)
...
no_name=('NT_PRXREG',),
# Cf. https://github.com/torvalds/linux/blob/master/include/uapi/linux/elf.h
...
NT_S390_TODCMP      = 0x302, # s390 time-of-day (TOD) clock comparator register
NT_S390_TODPREG     = 0x303, # s390 time-of-day (TOD) programmable register
...
# Cf. https://sourceware.org/ml/gdb-patches/2016-01/msg00277.html
...
SetConstants(
# Cf. http://man7.org/linux/man-pages/man5/elf.5.html
NT_GNU_ABI_TAG      = 1, # ABI version tag
NT_GNU_HWCAP        = 2, # Synthetic hwcap information.
NT_GNU_BUILD_ID     = 3, # Unique build ID as generated by the GNU ld --build-id option.
NT_GNU_GOLD_VERSION = 4, # GNU Gold linker version used.
)
...
no_name = ('STB_LOOS', 'STB_HIOS', 'STB_LOPROC', 'STB_HIPROC'),
)
...
no_name = ('STT_LOOS', 'STT_HIOS', 'STT_LOPROC', 'STT_HIPROC'),
)
...
no_name = ('DT_ENCODING', 'DT_LOPROC', 'DT_HIPROC',
...
R_386_32PLT        = 11,
R_386_TLS_GD_PLT   = 12,           # This relocation is handled as if it were a R_386_PLT32 relocation referencing the ___tls_get_addr() function
...
no_name = ('R_PARISC_LORESERVE','R_PARISC_HIRESERVE')
)
...
for k in constants:
    if k.startswith('R_'):
        if   k == 'R_390':  m = 'S390'
        elif k == 'R_IA64': m = 'IA_64'
        else:               m = k[2:]
        constants['R'][m] = constants[k]
